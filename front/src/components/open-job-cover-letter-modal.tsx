...
interface OpenJobCoverLetterModalProps {
    isOpen: boolean
    onClose: () => void
    matchId: string
}

export function OpenJobCoverLetterModal({ isOpen, onClose, matchId }: OpenJobCoverLetterModalProps) {
    ...
    const generateCoverLetter = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('https://resume.bhaikaamdo.com/api/v1/cover-letters/getOpenJobCoverletter', {
                method: 'POST',
                headers: {
            ...
            <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col p-0 glass-panel border-white/20 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)]">
                    ...
                    <div className="flex items-center justify-between">
                         <div className="space-y-1">
                            ...
                            <DialogDescription className="text-sm text-muted-foreground font-medium">
                                Review and refine your <span className="text-foreground font-bold italic underline decoration-primary">AI-curated</span> narrative.
                            </DialogDescription>
                         </div>
                        <div className="flex items-center gap-3">
                            <Button 
                                ...
                                className="rounded-xl h-11 px-6 font-bold shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95"
                            >
                    ...
                    {loading ? (
                        <div className="flex-1 flex items-center justify-center flex-col gap-6 py-24">
                            ...
                            <div className="text-center space-y-1">
                                <p className="text-lg font-bold">Synthesizing Voice</p>
                                <p className="text-xs text-muted-foreground uppercase tracking-widest font-black">Aligning background with job DNA…</p>
                            </div>
                        </div>
                    ) : (
                ...
                <div className="px-8 py-4 bg-white/5 border-t border-white/10 flex justify-between items-center">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">
                        Neural Engine v3.1 | Secure Output
                    </p>
